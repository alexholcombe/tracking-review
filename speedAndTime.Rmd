...
If the objects of an MOT display are moved faster and faster, eventually one can no longer track the targets. At very high speeds this is not surprising, as then one will run afoul of basic perceptual limits. As we will see, however, tracking falters at far lower speeds than expected from studies of motion perception. In addition, the maximum speed at which one can track is lower when there are more targets [@holcombeExhaustingAttentionalTracking2012] Apparently, the more resource one can allocate to a target, the faster one can track it.
