...
## Low-level and high-level temporal limits
...
Where does object tracking fit into this? A good starting point to consider is the ambiguous apparent motion depicted in the "higher-order motion" part of Figure \@ref(fig:slowFastBoxesArrows). If those two frames are alternated, one can see apparent motion clockwise or counter-clockwise. Which direction one sees depends on attention - one can actually choose to see the figure to rotate clockwise or to rotate counter-clockwise [@wertheimerExperimentelleStudienUber1912]. @verstratenLimitsAttentiveTracking2000 found that the maximum alternation rate at which this could be done was between 4 and 8 Hz. The temporal frequency here is the rate at which a dot is presented at a location.
In addition to this test of 2-frame apparent motion, they also presented unambiguous apparent motion by adding intermediate frames in between the the two original frames, in either the clockwise or counter-clockwise direction. This increased the maximum temporal frequency for each of three participants, but not to any higher than 8 Hz. A similar result was found for continuous motion of a grating, where temporal frequency reflects how often a bright (or dark) bar of the grating traverses any one location.

Another way to think about increasing temporal frequency is that it creates close encounters in time. That is, just as increasing the display density eventually impairs tracking due to spatial close encounters (crowding), a more dense display will also mean a higher temporal frequency, with an object and a distractor visiting the same spatial region in a short span of time.

, in analogy to the spatial close encounters (crowding)

the same way that increasing spatial density results in  
...
@holcombeSplittingAttentionReduces2013 and @roudaiaDifferentEffectsAging2017 replicated these results with the continuously-moving discs used in conventional MOT. These findings suggest an individuation limit wherein if a stimulus repeats at a particular location within a certain amount of time, about 120 ms if the limit is 8 Hz, the processes responsible for tracking fail. @vandegrindTemporalTransferProperties1973 suggested something similar when they coined the term "Gestalt fusion" to refer to how they saw the two phases of a flickering light as being simultaneously perceived when the flicker rate was above about 7 Hz.

For a continuously-moving array of stimuli, Figure \@ref(fig:temporalResltnSushi) depicts how this conception explains the dependence of object tracking on speed and density of an array of moving stimuli. At low stimulus speed and density the interval between stimuli occuping any one spatial location is long, so tracking succeeds (top panel). When one increases either speed (middle panel) or density (lower panel), the interval between visits to each location decreases. 
...
Quantitatively, the rate of stimulation of each location is the product of speed and density. For a circular array, for example, temporal frequency equals speed (in revolutions per second) times the number of discs in the circular array.
...
For that stimulus, temporal frequency is  

tested continuous motion with a 
hey also tested continuous motion rather than 2-frame apparent mo

If the discs were moving continuously rather than 

@verstratenLimitsAttentiveTracking2000 also increased the number


It falls into the slow group. The paper that measured ambiguous apparent motion ("higher-order motion") in Figure \@ref(fig:slowFastBoxesArrows) also measured tracking of a single bar of a continuously moving circular grating and found limits of 4-8 Hz, depending upon the participant, which was replicated by @holcombeSplittingAttentionReduces2013 and @roudaiaDifferentEffectsAging2017 with the discs used in conventional MOT.

and may be more basic than the ones that require feature identification.  Cite FujisakiNishida
...
An increase in close encounters isn't the only consequence of higher speeds in a standard MOT trial. At least, close encounters in *space* isn't the only consequence. There's another sort of close encounter, one that most people aren't familiar with, the *temporal* close encounter.
 ...
 it should be familiar in the context of stimuli that don't move but do change. 
 
which is the only kind most people are familiar with, aren't the only such phenomenon.

Close encounters aren't the only phenomenon that increase with speed in a standard MOT trial. At least, close encounters in *space*, which is the only kind most people are familiar with, aren't the only such phenomenon.
