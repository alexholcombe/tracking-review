...
@holcombeSplittingAttentionReduces2013 found a similar result (for 6 participants, a mean of 150 ms) using a circular array of discs rather than a grating. But how did they (and @verstratenLimitsAttentiveTracking2000) establish that this was due to temporal interference rather than spatial interference or to speed? The secret lies in the contrasting predictions for these three phenomena when object spacing and speed are varied. In @holcombeSplittingAttentionReduces2013 used four different inter-object spacings and many different speeds (speeds were adjusted by a staircase) to assess the speed threshold for each object spacing. If temporal interference affects performance, then a particular pattern of speed thresholds were predicted to emerge.

With a circular array of equally-spaced objects like that used by @holcombeSplittingAttentionReduces2013, the amount of time between objects traveling over any one location is given by the inverse of the product of the speed and the number of objects in the array. For example, if there are four objects in the array, and they are moving at 1.75 revolutions per second, then an object will cross any given location in the circular trajectory about every 140 milliseconds. Therefore, if there is temporal interference when objects are in a location within 140 milliseconds of each other, the speed threshold for eight objects in an array should be very different - 0.875 revolutions per second. This is indeed what was found for three different object number conditions.
...
The movie above is identical to the previous one except that there are 9 objects in each array instead of 3. Again try tracking the two objects that are initially white. In this case people find that as the objects accelerate, very quickly they feel that they can no longer track the objects. Note that with the use of 9 objects in an array, the display is still far below the spatial interference range, which should not be significant until the number of equidistant objects in an array exceeds 13 [@holcombeSplittingAttentionReduces2013, p.11; @toetTwodimensionalShapeSpatial1992; @pelliUncrowdedWindowObject2008].

## Temporal interference is resource-intensive

Not only did @holcombeSplittingAttentionReduces2013 add to the evidence of @verstratenLimitsAttentiveTracking2000 that there is a temporal limit on tracking, but also they found that that temporal limit was markedly worse when there are more targets to track. Specifically, the limit was `r round(1000/7)` ms for one target, about `r round(1000/4.2)` ms for two targets, and `r round(1000/2.6)` ms for three targets. The temporal limit can thus be described as resource-intensive, meaning that the more attentional resource that is allocated to each target, the higher the temporal limit.

This very large effect on temporal interference of the number of targets seems quite different from its effect on spatial interference. @holcombeObjectTrackingAbsence2014 found a trend for greater spatial interference with two targets compared to one, but the effect was statistically insignificant and in any case small relative to the performance cost of a second target (see section /@ref(spatialInterference)). So, what explains the dramatic effect of number of targets on the temporal limit?
...
ggplot(tibl, aes(x=targets,y=temporalLimit)) + geom_line( aes(color=factor(samplingInterval)) ) +
        geom_point(data=criticalIntervalsObserved) + 
