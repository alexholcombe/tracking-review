...
The data are plotted separately below because that is how they reported them.. I do not discuss or show the results they found for their old (greater than 60 years) group because that group included many outliers with very low performance. 
...
In each of the four datasets, one sees that the temporal limit decreases dramatically with increasing target load.
...
In section \@ref(whichAspects), we made the point that attentional tracking is a complex task - six factors were listed there that are likely to affect tracking performance. We also emphasized that a critical issue was resource-intensiveness, in part because the most resource-intensive factors are likely to be those that are most limiting the number of objects we can track.

. One of the most interesting things about our ability to track is its limited capacity.

the question of which factors are most resource-intensive.

From the pattern of performance with typical MOT displays, we could not know which constraining factor(s) might have the narrowest bottlenecks and thus be most affected by target load.
...
If it is appropriate to think of this rate as an individuation limit, then this lack of individuation might also explain the slow temporal limits reviewed in \@ref(Low-level and high-level temporal limits) above.
