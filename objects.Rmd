...
However, what might be the case is that once one *is* attending to an object, some process causes one to attend to the entire object, rather than just one or a few individual locations on it. This does seem to happen, and shows up in experiments where one end of a (static) object is cued, which results in facilitation not only for probes at that end of the object, but also at its other end. Most such research has been based on the cuing paradigm of @eglyShiftingVisualAttention1994a, wherein two static objects are presented and the end of one of them is cued. Subsequently, a stimulus to be identified is presented as part of the cued object or the other, uncued object. In the case of presentation on the cued object, this stimulus can be presented either in the same location as the cue, or on a different part of that object. The rationale is that if attention operates on entire objects, 

This suggests that when an object is cued, attention ends up occupying that entire object.
The results typically published are that participants are fastest and most accurate when the stimulus is presented in the same location as the cue, or on the same object albeit on a different part of that object. 

You may have noticed that I described

The outcome of interest is the difference in accuracy or response time between the 

Object-based attention has become a popular area of study, but rather than using tracking, they usually 


This pattern is not always observed, so this is not fully understood [@davisReversalObjectBased2005; @shomsteinObjectbasedAttentionStrength2008; @shomsteinObjectbasedAttentionSensory2002; @louIndividualDifferencesTemporal2020; @@francisExcessSuccessArticles2021], but perhaps the best explanation is that attention often does tend to spread to an entire object.
...
<!-- Cite Maechler, Cavanagh, & Tse somewhere -->
