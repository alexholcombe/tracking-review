...
I am aware of three theorists that have addressed the temporal interference results. @lovettSelectionEnablesEnhancement2019 have a hybrid parallel-serial model. Positions are updated in parallel, but 




Before I criticize the suggestions of @lovettSelectionEnablesEnhancement2019 and 
The parallel aspect of their model cannot explain this result, so they switch to serial .

They have a really weird account to explain the Holcombe rps and temporal frequency results. The idea is that because the targets move on circular trajectories, participants shift from parallel tracking to serial tracking (which in their model allows to take advantage of motion histories and do prediction), and this results in the decline in temporal frequency with load as well as the 2rps speed limit. But they never say what they'd predict if participants tracked in parallel. Also, my in-progress experiments with oscillating radii could help disprove this by making the trajectory too complicated to make it plausible that people are predicting it.

@liModelMultipleIdentity2019 "when objects move along the same trajectories (e.g., Holcombe & Chen, 2013) and/or are close to each other, high resolution information is required for discriminating the objects. Thus, tracking becomes more serial, and observers are inclined to switch attention and eye gaze from one object to another (Li, Oksama, & Hyönä, 2018a, 2018b; Meyerhoff et al., 2018)."
...
Most of these authors never consider the possibility of temporal interference, let alone an increase with target load.

A few have, but I find their solutions unsatisfying.
