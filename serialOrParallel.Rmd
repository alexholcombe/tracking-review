...
A serial account of tracking is that a process must switch from target to target to update targets' positions, one-by-one. This account makes a specific prediction for the temporal limits on tracking, after a few basic assumptions are made.
...
Other studies tend to find much slower sampling intervals:  `r round(1000/7,-1)` ms [@reFeatureBasedAttentionSamples2019]<!--They claimed 8 Hz in their paper but the figure shows the peak at 7 Hz-->, 142 ms [@vanrullenBlinkingSpotlightAttention2007], `r round(1000/7.5,-1)` ms <!--claimed 8 Hz but had equal power at 7 and 8-->[@fiebelkornReportRhythmicSampling2013], and `r round(1000/7.14,-1)` ms [@dugueAttentionSearchesNonuniformly2015]<!--said ~7 Hz but sampling was very coarse, one peak at 7.14--> <!--For the @reFeatureBasedAttentionSamples2019 paper, they wrote in their conclusion that sampling occurred at 8 Hz, but their figure shows a peak at 7 Hz, so I have used 7 Hz or `r round(1000/7,-1)` ms.--> A visual search study using neurophysiology in monkeys suggested a 44ms sampling interval @buschmanSerialCovertShifts2009

These other studies all used stationary objects or multiple locations that the participant was told to monitor, which perhaps explains the slower sampling intervals in many studies, although it is not clear why. Perhaps different tasks result in different sampling rates, and the process can occur more quickly for tracking as it requires sampling of position only. Another possibility is that the assumptions underlying the calculations in behavioral studies such as object tracking are wrong. If, for example, motion direction is used to guide the next position sampled, tracking could succeed with a slower sampling interval (but see section \@ref(beyondLocation)), more similar to those documented in most of the neural studies cited above.

In summary, then, there is some convergence of evidence that tracking involves a process that intermittently samples the targets, which can account for the dramatic worsening with target load observed by 
...
With non-circular trajectories, can capitalize on sensory memory?

"A single-focus attentional system with access to sensory memory does not need to update the registered positions of tracked objects every few video frames, but needs to update the registered positions at least once within the duration that object-traces persist in sensory memory."
