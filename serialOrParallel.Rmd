...
A particular sampling time specifies the line, so no parameters needed to be fit here. The lines fit the data fairly well, although it is not clear how impressive that should be at this point. Only three target loads were tested, for example, and it may be unlikely that young women differ as much from young men as is suggested by the Roudaia & Faubert data.

A particularly interesting issue is the temporal limit for 
...
a sampling interval per target of 60 ms is plotted together with that of a sampling interval of 90 ms.
