...
An additional issue for interpreting the relationship is that in principle, the temporal limit for covertly tracking a *single* target could be set by a different process than the serial switching hypothesized to set the limit for two and three targets, because with a single target, there is no need to switch attention around. However

a separate issue as it is not clear that it should fall on the same line as that for higher target numbers. When tracking a single target, there is no need to switch the position sampler to another object, and thus no need, it would seem, to have intermittent rather than continuous sampling. However, evidence from other paradigms suggests that even when only a single static location is relevant for attention, performance oscillates, in what has been called a "blinking spotlight" of attention [@vanrullenBlinkingSpotlightAttention2007; @fiebelkornReportRhythmicSampling2013]. In the tracking data plotted in \@ref(serialModelFit), it can be seen that the one-target limit is consistent with the other data points. Thus it may be that ON KNOWING WHERE OR WHEN BUT NOT WHAT. Is it just another paradigm for studying attention? No - hemisphere independence, where but not what.

Here, that the limit is consistent with the other data points suggests that even when only one target is present, it still is intermittently sampled, perhaps every 60 ms in the young male dataset.
