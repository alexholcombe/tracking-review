...
If someone says they are keeping track of something, what do they typically mean by that? A parent talking about keeping track of their family during an outing might mean is that they are continuously aware of where their two toddlers and their spouse is. They probably also mean that they are keeping track of which of them is where. Conventional laboratory multiple object tracking task do not test participants' knowledge of which target is where. The targets are typically all identical to each other and while participants have to report where the targets are, they do not indicate which is which.

This chapter is about what people know about objects when they track them. As we'll see the answer is that they know surprisingly little. We need to break the issue down, however, into two questions. The first question is about how the position updating aspect of tracking works - does it use differences between the distractors' and targets' features to help keep track of the targets? The second question is the extent to which the features of targets are available to conscious awareness.
