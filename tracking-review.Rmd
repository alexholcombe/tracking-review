...
```{r, echo=FALSE, out.width="100%", fig.cap="Spatial and temporal limits on covertly tracking one, two, and three targets. UPDATE THIS IMAGE WITH BETTER NUMBERS (SLIGHTLY LOWER SPEED LIMIT) IF REVIEWERS/EDITOR APPROVE OF IT"}
...
A human brain can be thought of as made up of two brains, a left and a right, just as we have two hand and two eyes. This however, is a misleading comparison, because for many functions, our two hemispheres are much more tightly integrated than are our left and right hands.
...
ASK AMANDA FOR FEEDBACK 

While the left and right hemispheres have substantial processing differences at the neural level, often this doesn't manifest in large behavioral differences because of the cross-hemisphere integration that can occur prior to a behavioral response. The functional independence of the two hemispheres for multiple object tracking, then, gives it a greater potential to show hemifield *differences* in performance, not just independence. Differences are sometimes observed, as reviewed below, but it should be kept in mind that typically these differences are not large.
...
The situation is clearly quite complex, but it remains possible that any right hemifield advantage for tracking is caused by the double processing of the right hemifield by parietal cortex, while the left visual field advantage for other tasks reflects other specializations of the right parietal lobe.
