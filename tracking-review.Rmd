...
Our minds show extraordinary limitations on how much visual information they can extensively process, in spite of massive parallel processing at our visual front ends. Studies of our ability to track a restricted number of moving objects have revealed that our capacity for updating the representations of objects is more limited than indicated by the studies with static stimuli that predominate in the attentional literature. The ability we have to track multiple objects is mediated by processes specific to each cerebral hemisphere, such that each hemifield has its own capacity limit. However, tracking can also benefit from more cognitive processes not specific to each hemifield, and recommendations are made to improve methodology so that the relative role of these two kinds of processes can be distinguished. We are far from fully understanding how attention follows multiple moving objects, but there are plenty of directions to go in for new discoveries.
...
```{r dmtemp, echo=FALSE, out.width="100%", fig.cap="A demonstration of the multiple object tracking (MOT) task, created by Jiri Lukavsky."}
#Work-around to make GIFs (but not .mov) work by avoiding including them in non-html outputs!
#mp4s don't work in R's html viewer that pops up when you build the book, but does work in a real web browser.
#https://stackoverflow.com/questions/64038037/can-i-conditionally-exclude-some-elements-code-blocks-from-rendering-to-the-pd
if(knitr::is_html_output()) knitr::include_url("movies/MOTmovies/connectedTargets/MOTtrackingEndOfLinesVeryDifficultScholl.mp4") else knitr::include_graphics("movies/MOTmovies/MOTdemoLukavsky.png")
#, height = "250px"
# Jiri uploaded it to https://www.youtube.com/embed/lAQM4QJRYV8
```
