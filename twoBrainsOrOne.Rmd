...
```
calculateIndependenceOnlyOneBaselineConfiguration

However, using trajectories that were not circular @hudsonHemifieldEffectsMultiple2012 found partial independence, although they used a rather different task (multiple identity tracking, which is discussed in \@ref(beyondLocation)). `r round(100*HudHoweLittleMITindependence)`%

`r round(100*HudHoweLittleMOTindependence)`%
