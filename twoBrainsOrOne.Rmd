...
The level of hemispheric independence for a task can be quantified from the advantage provided by presenting the relevant stimuli in distinct hemifields. If adding a second stimulus to a hemifield reduces performance by, say, 20 percentage points, but adding that second stimulus to the other hemifield instead reduces performance by only 5 percentage points, then we can say that there is (20-5) / 20 = 75% hemifield independence. It is not quite this simple, in that one needs to control for chance performance, but the previous sentence illustrates the principle.
