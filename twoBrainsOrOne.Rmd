...
The degree of hemispheric independence for a task can be quantified. If adding a second stimulus to a hemifield reduces performance by, say, 20 percentage points, but adding that stimulus to the other hemifield reduces performance by only 5 percentage points, one might say that there is (20-5) / 20 = 75% hemifield independence. A proper calculation of independence is not quite this simple, in that one needs to correct for the accuracy one can achieve by guessing.

When [this is done](https://github.com/alexholcombe/tracking-review/blob/main/twoBrainsOrOne.Rmd), the level of independence shown is extraordinary: `r round(100*alvarezCavanaghE1Independence)`% independence in one of the @alvarezIndependentResourcesAttentional2005 experiments, and `r round(100*alvarezCavanaghE3Independence)`% in the other. 

@alvarezIndependentResourcesAttentional2005 themselves did not report these figures. They calculated expected performance if the hemifields are in fact completely independent, and found that performance was not statistically significantly worse than that figure. They then suggested that there was complete independence, but this is based on the very common fallacy of concluding a null hypothesis is true when the evidence does not reject it at a p<.05 level [@aczelQuantifyingSupportNull2018]. In this case, for the null hypothesis they used their *conclusion* of complete independence, and then affirmed the conclusion on the basis of not finding much evidence against it.

The important point, however, is that their data do suggest strong hemispheric independence. Subsequent work by @chenResourceDemandsObject2013 <!--Didn't compare one to two targets in hemifield but did compare two targets in a hemifield to four targets across two hemifields--> supported the finding, as did  @holcombeExhaustingAttentionalTracking2012, who used speed thresholds to calculate 75% independence. <!--Calculated two-target prediction, and was about 75% independent, but plenty of noise non-significant speed threshold decreases of c(1.67,1.44,1.7,1.63) 1.98rps->1.85 --> also found high levels of independence.  @shimNumberAttentionalFoci2010<!--Figure 1--> also found a substantial bilateral advantage. 

This result has not always replicated [@shimSpatialSeparationTargets2008] <!--performance in all the conditions involved in their comparison were greater than 85% correct, including many low speeds where performance was near ceiling -->  <!--For most of these tasks, but not @shimNumberAttentionalFoci2010, more targets was confounded with having to make more responses and remember said responses at the end of the trial, so some of the residual cost could be due to that being unitary rather than hemifield-specific.-->, but the successful replications, and the substantial statistical power associated with them (evident in their error bars) indicate that at least in some circumstances, tracking does occur mostly independently in the two hemispheres. @shimSpatialSeparationTargets2008 suggested that the reason they did not find evidence for hemifield independence is that they used two targets, whereas the original @alvarezIndependentResourcesAttentional2005 report of hemifield independence used four targets. However, this is unlikely to be the reason for the discrepancy because @holcombeExhaustingAttentionalTracking2012 also used two targets and did find independence. It appears that the @shimSpatialSeparationTargets2008 data may have been affected by performance being near ceiling. Another difference is that the papers findings independence used circular trajectories. However, using trajectories that were not circular @hudsonHemifieldEffectsMultiple2012 found partial independence, although they used a rather different task.
